...
import { useIsMobile } from "@supagen/ui/hooks/use-mobile";
...
function Sidebar({
  ...
  if (isMobile) {
    return (
      <div
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex h-svh w-(--sidebar-width) flex-col bg-sidebar text-sidebar-foreground transition-transform duration-200 ease-linear md:hidden",
...
function SidebarTrigger({
  ...
  return (
    <button
      className={cn(
        "inline-flex size-7 items-center justify-center rounded-lg text-muted-foreground outline-none transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-3 focus-visible:ring-sidebar-ring/40",
        className,
      )}
      ...
      {...props}
    >
      <MaterialIcon name="left_panel_close" />
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-3 overflow-hidden rounded-lg py-2.5 px-3 text-left text-[13px] font-medium outline-hidden ring-sidebar-ring transition-[padding,gap] duration-200 ease-linear hover:bg-muted hover:text-foreground focus-visible:ring-2 active:bg-muted active:text-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-muted data-[state=open]:hover:text-foreground group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:gap-0! group-data-[collapsible=icon]:px-0! [&>span:last-child]:truncate [&_[data-slot=material-icon]]:shrink-0 [&>svg]:shrink-0 [&>svg]:size-4",
  {
    variants: {
      variant: {
  ...
  return (
    <Comp
      className={cn(
        ...
        "group-data-[collapsible=icon]:hidden",
        showOnHover &&
          "data-[state=open]:opacity-100 md:opacity-0 group-focus-within/menu-item:opacity-100 group-hover/menu-item:opacity-100 peer-data-[active=true]/menu-button:text-sidebar-accent-foreground",
        className,
      )}
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      className={cn(
        "mx-0 flex min-w-0 flex-col gap-0.5 border-l-0 py-0.5 pl-4",
        "group-data-[collapsible=icon]:hidden",
        className,
      )}
