...
describe(ListModelCatalogModelsUseCase.name, () => {
  ...
  it("loads a single model and enriches capabilities from model endpoints", async () => {
    const source = createSource();
    ...
    source.getModelCapabilities.mockResolvedValue(endpointCapabilities);
    const useCase = new ListModelCatalogModelsUseCase(source);
    ...
    expect(source.getModelCapabilities).toHaveBeenCalledWith(models[0]);
  });
  ...
  function createSource(): jest.Mocked<ModelCatalogSource> {
    return {
      ...
      getModelCapabilities: jest.fn(async (model) => model.capabilities),
    };
  }
});
